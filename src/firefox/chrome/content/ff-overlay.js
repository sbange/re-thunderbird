...
var markdown_here = {
  ...
  onMenuItemCommand: function(e) {
    ...
    // TODO sb remove me!
    window.document.addEventListener( "compose-send-message", function() {self.log('Document sent...')}, true );
    // \TODO
  ...
  markdownRender: function(targetDocument, html, callback) {
    ...
    OptionsStore.get(function(prefs) {
      ...
      request.onload = function(aEvent) {
        responseHtml = responseHtml + aEvent.target.responseText;
      };
      request.open("POST", url, false);
      request.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
      request.send(data);
      // image tag needs attribute moz-do-not-send="true" or else thunderbird will attach image inline;
      html = html + '<br><br>' + responseHtml;
      // \TODO
      callback(html, prefs['main-css'] + prefs['syntax-css']);
    });
    /*
    OptionsStore.get(function(prefs) {
      callback(
